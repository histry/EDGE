...
Unified Planner v3 adds EDGE_RAG_SUMMARY_MODE=mean|temporal.
- mean: default, sequence-level summary expanded over all frames.
- temporal: local triangular windows around EDGE_RAG_SUMMARY_MID_FRAMES.
"""
...
def _build_temporal_rag_summary(unit_paths: Sequence[str], frames: Sequence[int], batch_size: int, seq_len: int, dim: int, device, dtype) -> torch.Tensor:
    ...
    mode = _env_str("EDGE_RAG_SUMMARY_MODE", "mean")
    cache_key = (tuple(valid_paths), tuple(int(x) for x in frames), int(batch_size), int(seq_len), int(dim), str(device), str(dtype), int(radius), mode)
    ...
    if mode == "temporal" and frames and len(frames) == len(vecs):
...
def _maybe_attach_rag_summary(diffusion, cond: dict, shape: Optional[tuple]) -> dict:
    if not isinstance(cond, dict) or cond.get("rag_summary", None) is not None:
        return cond
    ...
    mode = _env_str("EDGE_RAG_SUMMARY_MODE", "mean")
    rag = _build_temporal_rag_summary(unit_paths, frames, batch_size, seq_len, dim, device, dtype)
    cond = dict(cond)
    cond["rag_summary"] = rag
    print("✅ V9 RAG summary attached to inference cond: " f"units={len(unit_paths)}, mode={mode}, frames={frames if frames else 'mean'}, shape={tuple(rag.shape)}")
    return cond
