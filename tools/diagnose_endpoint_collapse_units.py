...
def compute(pred: np.ndarray, gt: Optional[np.ndarray]) -> Dict[str, object]:
    ...
    dist_start = safe_norm(feat - start[None, :], axis=-1)
    dist_end = safe_norm(feat - end[None, :], axis=-1)
    start_end_dist = float(safe_norm(end - start, axis=-1))

    # When start/end are close, use dynamic scale from frame deltas.
    ...
    # Compare to GT temporal progress if available.
    ...
    return {
        "endpoint_collapse_bad": endpoint_collapse_bad,
...
def main() -> None:
    ...
    for p in files:
        ...
        try:
            ...
            row = {
                ...
                "endpoint_collapse_bad": True,
                "bad_reasons": f"error:{exc}",
            }
        rows.append(row)

    fieldnames = [
        "unit", "file", "gt_file",
        "endpoint_collapse_bad", "bad_reasons",
        "end_reach_frame", "end_reach_frac", "end_hold_ratio",
        "eps_end", "start_end_dist", "mean_delta", "max_delta",
        "cum_motion_25", "cum_motion_50", "cum_motion_75",
        "gt_cum_motion_25", "gt_cum_motion_50", "gt_cum_motion_75",
        "progress_l1_vs_gt",
        "top1_delta_share", "top3_delta_share", "top5_delta_share",
    ]
    ...
    for r in rows:
        print(
            f"{r.get('unit')},"
            f"{r.get('endpoint_collapse_bad')},"
            f"{r.get('bad_reasons','')},"
            f"{float(r.get('end_reach_frac', 0) or 0):.3f},"
            f"{float(r.get('end_hold_ratio', 0) or 0):.3f},"
            f"{float(r.get('cum_motion_25', 0) or 0):.3f},"
            f"{float(r.get('cum_motion_50', 0) or 0):.3f},"
            f"{float(r.get('top1_delta_share', 0) or 0):.3f},"
            f"{float(r.get('top3_delta_share', 0) or 0):.3f},"
            f"{float(r.get('progress_l1_vs_gt', 0) or 0):.3f}"
        )
