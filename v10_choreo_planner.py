...
Unified Planner v3 goals:
1. Manual and auto modes use the same Unit representation.
2. Step 1 / 3 / 4 are config variants of one planner.
3. Beam search provides transition-aware global path search for multi-unit planning.
"""
...
def build_config_from_env(num_frames: int = 150) -> PlannerConfig:
    ...
    return PlannerConfig(
        ...
        manual_units=parse_csv(os.environ.get("EDGE_V10_MANUAL_UNITS", "")),
        manual_mid_poses=parse_csv(os.environ.get("EDGE_V10_MANUAL_MID_POSES", "")),
        search_method=env_str("EDGE_V10_SEARCH_METHOD", "beam" if mode == "auto_multiunit" else "greedy").lower(),
...
def score_unit(unit: np.ndarray, index: int = -1, source_key: str = "", source_index: int = -1, weights: Optional[Dict[str, float]] = None) -> UnitScore:
    unit = as_unit_t151(unit)
    weights = weights or {
        "upper": env_float("EDGE_V10_SCORE_UPPER_W", 0.45),
        "turn": env_float("EDGE_V10_SCORE_TURN_W", 0.15),
    ...
    score = float(weights.get("upper", 0.45)) * upper_activity + float(weights.get("turn", 0.15)) * turning + float(weights.get("diversity", 0.20)) * pose_diversity - float(weights.get("contact", 0.08)) * contact_change - float(weights.get("root_penalty", 0.12)) * root_speed - float(weights.get("lower_penalty", 0.05)) * max(0.0, lower_activity - upper_activity)
    unit_id = f"{source_key}:{int(source_index)}" if source_key else f"unit_{int(index)}"
    return UnitScore(index=int(index), score=float(score), emission_score=float(score), upper_activity=float(upper_activity), lower_activity=float(lower_activity), root_speed=float(root_speed), spatial_range=float(spatial_range), turning=float(turning), pose_diversity=float(pose_diversity), contact_change=float(contact_change), source_key=str(source_key), source_index=int(source_index), unit_id=unit_id)
...
def load_units_from_npz(rag_db: str, max_units: Optional[int] = None) -> Tuple[List[np.ndarray], List[Dict[str, Any]]]:
    ...
    for key, arr in arrays:
        for i in range(arr.shape[0]):
            ...
            meta.append({"source_key": key, "source_index": int(i), "global_index": int(global_index), "unit_id": f"{key}:{int(i)}", "alt_unit_id": f"unit_{int(global_index):04d}"})
            global_index += 1
...
class RAGUnitDB:
    ...
    def resolve_unit_ids(self, unit_ids: Sequence[str]) -> Tuple[List[np.ndarray], List[Dict[str, Any]]]:
        resolved_units, resolved_meta = [], []
        id_to_index: Dict[str, int] = {}
        for i, m in enumerate(self.meta):
            source_key = str(m.get("source_key", "")); source_index = int(m.get("source_index", i)); global_index = int(m.get("global_index", i))
            keys = {str(global_index), f"unit_{global_index}", f"unit_{global_index:04d}", f"{source_key}:{source_index}", f"{source_key}:{source_index:04d}", str(m.get("unit_id", "")), str(m.get("alt_unit_id", ""))}
            for key in keys:
                if key:
                    id_to_index[key] = i
        for raw in unit_ids:
            token = str(raw).strip(); idx = id_to_index.get(token)
            if idx is None:
            ...
            if idx is None:
                ...
                raise KeyError(f"Cannot resolve manual unit id {token!r}. Use global index like 1042/unit_1042 or source_key:index. Examples: {examples}")
            meta = dict(self.meta[idx]); meta["requested_unit_id"] = token
            resolved_units.append(self.units[idx]); resolved_meta.append(meta)
...
class UnifiedChoreoPlanner:
    def __init__(self, config: PlannerConfig, rag_db: Optional[str] = None):
        self.config = config
        self.rag_db = rag_db
        self.db = RAGUnitDB(rag_db, max_units=env_int("EDGE_V10_MAX_RAG_UNITS", 20000)) if rag_db else None
    def _score_all(self) -> List[UnitScore]:
        ...
        for i, unit in enumerate(self.db.units):
            m = self.db.meta[i]
            s = score_unit(unit, index=i, source_key=str(m.get("source_key", "")), source_index=int(m.get("source_index", i)), weights=self.config.score_weights)
            if s.upper_activity >= self.config.min_upper_activity and s.root_speed <= self.config.max_root_speed:
                scores.append(s)
        if not scores:
            for i, unit in enumerate(self.db.units):
                m = self.db.meta[i]
                scores.append(score_unit(unit, index=i, source_key=str(m.get("source_key", "")), source_index=int(m.get("source_index", i)), weights=self.config.score_weights))
    ...
    def _annotate_path(self, indices: Sequence[int], base_scores: Dict[int, UnitScore], start_pose: Optional[np.ndarray], end_pose: Optional[np.ndarray], path_score: float = 0.0) -> List[PlannedUnit]:
        ...
        for rank, idx in enumerate(indices):
            unit = self.db.units[idx]; base = base_scores[idx]; s = UnitScore(**asdict(base))
            if rank == 0:
                s.entry_cost = pose_distance(start_pose, unit[0]); s.transition_cost = float(self.config.transition_weights.get("start", 0.35)) * s.entry_cost
            else:
                s.entry_cost = pose_distance(prev_unit[-1], unit[0]); s.transition_cost = transition_cost(prev_unit, unit, self.config.transition_weights)
            if rank == len(indices) - 1:
                s.exit_cost = pose_distance(unit[-1], end_pose)
            else:
                next_unit = self.db.units[indices[rank + 1]]; s.exit_cost = pose_distance(unit[-1], next_unit[0])
            s.transition_score = s.score - s.transition_cost; s.path_score = float(path_score)
            meta = dict(self.db.meta[idx]); meta["rank"] = rank + 1
            planned.append(PlannedUnit(unit=unit, score=s, meta=meta)); prev_unit = unit
        return planned
    def _select_manual_units(self, start_pose: Optional[np.ndarray], end_pose: Optional[np.ndarray]) -> List[PlannedUnit]:
        ...
        for unit, m in zip(units, meta):
            idx = int(m["global_index"]); indices.append(idx)
            base_scores[idx] = score_unit(unit, index=idx, source_key=str(m.get("source_key", "")), source_index=int(m.get("source_index", idx)), weights=self.config.score_weights)
    ...
    def _select_greedy(self, start_pose: Optional[np.ndarray], end_pose: Optional[np.ndarray]) -> List[PlannedUnit]:
        ...
        base_list = self._score_all(); base_scores = {s.index: s for s in base_list}
        selected, used_sources, prev_unit = [], set(), None
        for slot in range(self.config.count):
            scored_slot = []; is_last = slot == self.config.count - 1
            for base in base_list:
                if base.index in selected: continue
                unit = self.db.units[base.index]
                cost = float(self.config.transition_weights.get("start", 0.35)) * pose_distance(start_pose, unit[0]) if prev_unit is None else transition_cost(prev_unit, unit, self.config.transition_weights)
                if is_last:
            ...
            if not scored_slot: break
            _, best_idx = max(scored_slot, key=lambda x: x[0])
            selected.append(best_idx); used_sources.add(base_scores[best_idx].source_key); prev_unit = self.db.units[best_idx]
        return self._annotate_path(selected, base_scores, start_pose, end_pose, path_score=float(sum(base_scores[i].score for i in selected)))
    def _select_beam(self, start_pose: Optional[np.ndarray], end_pose: Optional[np.ndarray]) -> List[PlannedUnit]:
        ...
        base_list = self._score_all()[: max(1, self.config.top_k)]; base_scores = {s.index: s for s in base_list}
        if not base_list: raise RuntimeError("No V10 unit candidates found.")
        beams: List[Tuple[float, List[int], set[str]]] = [(0.0, [], set())]
        for slot in range(self.config.count):
            new_beams = []; is_last = slot == self.config.count - 1
            for cur_score, path, used_sources in beams:
                prev_unit = self.db.units[path[-1]] if path else None
                for cand in base_list:
                    idx = cand.index
                    if idx in path: continue
                    unit = self.db.units[idx]; step_score = cand.score
                    ...
                    next_used = set(used_sources); next_used.add(cand.source_key)
        ...
        if not beams: raise RuntimeError("Beam search produced no valid path.")
    ...
    def plan(self, out_prefix: str, start_pose_path: str = "", end_pose_path: str = "") -> Dict[str, Any]:
        start_pose = load_151_pose(start_pose_path); end_pose = load_151_pose(end_pose_path)
        if self.config.manual_units:
            planned_units = self._select_manual_units(start_pose, end_pose)
        elif self.config.manual_mid_poses:
            return save_manual_pose_plan(self.config, out_prefix)
        elif self.config.search_method == "beam":
            planned_units = self._select_beam(start_pose, end_pose)
        else:
            planned_units = self._select_greedy(start_pose, end_pose)
        return save_planned_units(planned_units, self.config.mid_frames, out_prefix, self.config, self.rag_db, start_pose_path, end_pose_path)


def _score_parts_from_score(score: UnitScore) -> Dict[str, Any]:
    return {"phase": "v10_unified_planner", "unit_id": score.unit_id, "source_key": score.source_key, "source_index": int(score.source_index), "global_index": int(score.index), "emission_score": float(score.emission_score), "motion_energy_norm": float(score.score), "transition_score": float(score.transition_score), "path_score": float(score.path_score), "entry_cost": float(score.entry_cost), "exit_cost": float(score.exit_cost), "transition_cost": float(score.transition_cost), "upper_activity": float(score.upper_activity), "lower_activity": float(score.lower_activity), "root_speed": float(score.root_speed), "spatial_range": float(score.spatial_range), "turning": float(score.turning), "pose_diversity": float(score.pose_diversity), "contact_change": float(score.contact_change)}


def _keyframe_from_item(item: Dict[str, Any]) -> Dict[str, Any]:
    score_parts = item.get("score_parts", {}) or {}
    return {"frame": int(item.get("frame", 0)), "segment_id": int(item.get("rank", -1)), "phase": str(item.get("phase", "v10_unified_planner")), "source": item.get("unit_path", item.get("pose_path", "")), "score": float(score_parts.get("transition_score", score_parts.get("emission_score", 0.0))), "score_parts": score_parts}


def _attach_legacy_keyframes(plan: Dict[str, Any]) -> Dict[str, Any]:
    items = plan.get("items", []) or []
    keyframes = [_keyframe_from_item(item) for item in items]
    plan["keyframes"] = keyframes; plan["auto_keyframes"] = keyframes
    return plan


def save_planned_units(planned_units: Sequence[PlannedUnit], frames: Sequence[int], out_prefix: str, config: PlannerConfig, rag_db: Optional[str] = None, start_pose_path: str = "", end_pose_path: str = "") -> Dict[str, Any]:
    if len(planned_units) != len(frames):
        raise ValueError(f"planned_units count {len(planned_units)} != frames count {len(frames)}")
    out_prefix_path = Path(out_prefix); out_prefix_path.parent.mkdir(parents=True, exist_ok=True)
    mid_paths, unit_paths, unit_ids, items = [], [], [], []
    for k, (planned, frame) in enumerate(zip(planned_units, frames), start=1):
        unit = as_unit_t151(planned.unit); score = planned.score; meta = planned.meta
        mid_idx = int(round((len(unit) - 1) / 2)); pose = unit[mid_idx].astype(np.float32)
        ...
        np.save(pose_path, pose); np.save(unit_path, unit.astype(np.float32))
        mid_paths.append(str(pose_path)); unit_paths.append(str(unit_path)); unit_ids.append(str(score.unit_id))
        score_parts = _score_parts_from_score(score)
        items.append({"rank": k, "frame": int(frame), "phase": "v10_unified_planner", "pose_path": str(pose_path), "unit_path": str(unit_path), "unit_id": str(score.unit_id), "requested_unit_id": str(meta.get("requested_unit_id", "")), "source_key": str(meta.get("source_key", score.source_key)), "source_index": int(meta.get("source_index", score.source_index)), "global_index": int(meta.get("global_index", score.index)), "score_parts": score_parts})
    plan = {"version": "v10_unified_planner_v3", "mode": config.mode, "search_method": config.search_method, "mid_poses": mid_paths, "mid_pose_frames": [int(f) for f in frames], "unit_paths": unit_paths, "unit_ids": unit_ids, "items": items, "rag_db": str(rag_db or ""), "start_pose_path": str(start_pose_path or ""), "end_pose_path": str(end_pose_path or ""), "transition_aware": True, "v9_rag_summary_unit_paths": unit_paths, "planner_config": asdict(config), "conditions": {"use_rag_summary": bool(unit_paths), "rag_summary_mode": config.rag_summary_mode, "use_unit_prior": env_bool("EDGE_UNIT_SOFT_PRIOR", True), "unit_prior_strength": env_float("EDGE_UNIT_PRIOR_STRENGTH", 0.012), "mid_keyframe_strength": env_float("EDGE_V10_MID_STRENGTH", 0.035)}}
    ...
    print("✅ V10 Unified planner selected units: " + ", ".join(f"{uid}@f{frame}" for uid, frame in zip(unit_ids, frames)))
    return plan
...
def save_manual_pose_plan(config: PlannerConfig, out_prefix: str) -> Dict[str, Any]:
    ...
    if len(paths) != len(config.mid_frames):
        raise ValueError(f"Manual pose count {len(paths)} must match frame count {len(config.mid_frames)}.")
    for p in paths:
        if not Path(p).exists(): raise FileNotFoundError(f"Manual mid pose not found: {p}")
    out_prefix_path = Path(out_prefix); out_prefix_path.parent.mkdir(parents=True, exist_ok=True)
    items = [{"rank": i + 1, "frame": int(frame), "phase": "v10_manual_pose_legacy", "pose_path": path, "unit_path": "", "unit_id": "", "source_key": "manual_pose", "source_index": -1, "global_index": -1, "score_parts": {"phase": "v10_manual_pose_legacy", "warning": "No unit_paths: V9 rag_summary and DCT unit prior are unavailable in legacy manual-pose mode.", "emission_score": 0.0, "transition_score": 0.0, "entry_cost": 0.0, "exit_cost": 0.0}} for i, (path, frame) in enumerate(zip(paths, config.mid_frames))]
    plan = {"version": "v10_unified_planner_v3", "mode": "manual_pose_legacy", "search_method": "manual_pose", "mid_poses": paths, "mid_pose_frames": [int(f) for f in config.mid_frames], "unit_paths": [], "unit_ids": [], "items": items, "transition_aware": False, "planner_config": asdict(config), "conditions": {"use_rag_summary": False, "use_unit_prior": False, "warning": "Prefer EDGE_V10_MANUAL_UNITS for fair Manual vs Auto comparison."}}
    plan = _attach_legacy_keyframes(plan)
    plan_path = out_prefix_path.parent / f"{out_prefix_path.name}_v10_plan.json"
    with open(plan_path, "w", encoding="utf-8") as f: json.dump(plan, f, ensure_ascii=False, indent=2)
    plan["plan_path"] = str(plan_path)
...
def plan_choreo_from_rag_db(rag_db: str, out_prefix: str, config: Optional[PlannerConfig] = None, start_pose_path: str = "", end_pose_path: str = "") -> Dict[str, Any]:
...
def plan_upperdance_from_rag_db(rag_db: str, out_prefix: str, num_frames: int = 150, count: int = 2, frames: Optional[Sequence[int]] = None, start_pose_path: str = "", end_pose_path: str = "") -> Dict[str, Any]:
    config = build_config_from_env(num_frames=num_frames)
    if frames is not None: config.mid_frames = [int(x) for x in frames]
    elif len(config.mid_frames) != int(count): config.mid_frames = parse_frames("", count=int(count), num_frames=num_frames)
    return plan_choreo_from_rag_db(rag_db=rag_db, out_prefix=out_prefix, config=config, start_pose_path=start_pose_path, end_pose_path=end_pose_path)


def plan_manual_from_env(out_prefix: str, num_frames: int = 150) -> Optional[Dict[str, Any]]:
    config = build_config_from_env(num_frames=num_frames)
    if not config.manual_units and not config.manual_mid_poses: return None
    rag_db = os.environ.get("EDGE_V10_RAG_DB") or os.environ.get("RAG_DB", "")
    if config.manual_units and not rag_db: raise RuntimeError("EDGE_V10_MANUAL_UNITS requires EDGE_V10_RAG_DB or RAG_DB.")
    planner = UnifiedChoreoPlanner(config=config, rag_db=rag_db or None)
