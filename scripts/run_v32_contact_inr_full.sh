...
DATA="$RUN_ROOT/v32_transition_dataset.npz"
AUDIT="$RUN_ROOT/v32_contact_dataset_audit.json"
...
python tools/audit_v32_contact_dataset.py \
  ...
  --min_contact_rate "${V32_MIN_CONTACT_RATE:-0.005}" \
  --max_contact_rate "${V32_MAX_CONTACT_RATE:-0.95}"

# ------------------------------------------------------------------
# 2. Train continuous INR, contact fine-tuning, then latent diffusion.
# ------------------------------------------------------------------
if [[ "${V32_TRAIN:-1}" == "1" ]]; then
  python train_v27_transition_diffusion.py \
    ...
    --amp "${V32_AMP:-1}" \
    --fps "${V26_FPS:-30}" \
    --w_contact_bce "${V32_W_CONTACT_BCE:-0.35}" \
    --w_contact_skate "${V32_W_CONTACT_SKATE:-1.20}" \
    --w_contact_height "${V32_W_CONTACT_HEIGHT:-0.45}" \
    --w_foot_penetration "${V32_W_FOOT_PENETRATION:-0.80}" \
    --w_swing_clearance "${V32_W_SWING_CLEARANCE:-0.12}" \
    --w_contact_temporal "${V32_W_CONTACT_TEMPORAL:-0.08}" \
    --w_contact_binary "${V32_W_CONTACT_BINARY:-0.02}" \
    --seed "${V32_SEED:-20260610}"
...
# ------------------------------------------------------------------
# 4. Evaluate V32 and the deterministic baseline.
# ------------------------------------------------------------------
evaluate_directory() {
  local directory="$1"
  local label="$2"
