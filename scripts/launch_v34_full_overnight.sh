...
export RUN_ID="${RUN_ID:-v34_full_overnight_$(date +%Y%m%d_%H%M%S)}"
export RUN_ROOT="${RUN_ROOT:-output/$RUN_ID}"
mkdir -p "$RUN_ROOT"
echo "$RUN_ROOT" > output/LATEST_V34_OVERNIGHT_LAUNCH.txt

for path in \
  "$V26_INDEX_JSON" "$V26_DURATION_INDEX_NPZ" \
  "$V26_ROUTER_CKPT" "$V26_V23_CKPT" "$V26_PLANNER_CKPT" \
  "$V26_START_POSE" "$V27_HYPERBOLIC_CKPT" \
  "$V26_HIERARCHY_INDEX_NPZ" "$V33_EVENT_CONTACT_CACHE" \
  "$V34_DATASET"; do
  [[ -f "$path" ]] || { echo "[PRECHECK ERROR] Missing $path" >&2; exit 2; }
done
