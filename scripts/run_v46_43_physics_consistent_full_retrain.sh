...
# V46.43 loophole fixes.
export V46_43_EARLY_ABORT_KBO_SMOOTH_SIGMA="${V46_43_EARLY_ABORT_KBO_SMOOTH_SIGMA:-1.35}"
export V46_43_EARLY_ABORT_KBO_RELAX="${V46_43_EARLY_ABORT_KBO_RELAX:-3.0}"
export V46_43_MSA_HIGH_ENERGY_SCALE="${V46_43_MSA_HIGH_ENERGY_SCALE:-0.22}"
export V46_43_MSA_DYNAMIC_ATTENUATION="${V46_43_MSA_DYNAMIC_ATTENUATION:-0.75}"
export V46_43_MSA_ROOT_SPEED_RELAX_THRESH="${V46_43_MSA_ROOT_SPEED_RELAX_THRESH:-0.045}"
export V46_43_MSA_MIN_WEIGHT="${V46_43_MSA_MIN_WEIGHT:-0.05}"
export V46_43_ENABLE_HN_DPO_FINETUNE="${V46_43_ENABLE_HN_DPO_FINETUNE:-${V46_42_ENABLE_HN_DPO_FINETUNE:-0}}"
export V46_43_HN_DPO_STEPS="${V46_43_HN_DPO_STEPS:-1500}"
export V46_43_HN_DPO_KINETIC_WEIGHT="${V46_43_HN_DPO_KINETIC_WEIGHT:-0.18}"
export V46_43_HN_DPO_KINETIC_FLOOR_RATIO="${V46_43_HN_DPO_KINETIC_FLOOR_RATIO:-0.80}"


# V46.43 physics-consistent corrections.
export V46_43_EARLY_ABORT_LOWPASS_SIGMA="${V46_43_EARLY_ABORT_LOWPASS_SIGMA:-2.25}"
export V46_43_EARLY_ABORT_RELAX="${V46_43_EARLY_ABORT_RELAX:-4.0}"
export V46_43_EARLY_ABORT_CONSECUTIVE_FATAL="${V46_43_EARLY_ABORT_CONSECUTIVE_FATAL:-2}"
export V46_43_EARLY_ABORT_ALLOW_DERIVATIVE_ONLY_FATAL="${V46_43_EARLY_ABORT_ALLOW_DERIVATIVE_ONLY_FATAL:-0}"
export V46_43_MSA_LEAP_SPEED_THRESH="${V46_43_MSA_LEAP_SPEED_THRESH:-0.070}"
export V46_43_MSA_LEAP_DILATE="${V46_43_MSA_LEAP_DILATE:-4}"
export V46_43_MSA_LEAP_MIN_GATE="${V46_43_MSA_LEAP_MIN_GATE:-0.0}"
export V46_43_MSA_CORRECTION_LOWPASS_SIGMA="${V46_43_MSA_CORRECTION_LOWPASS_SIGMA:-10.0}"
export V46_43_MSA_MAX_CORRECTION_VEL_MPF="${V46_43_MSA_MAX_CORRECTION_VEL_MPF:-0.006}"
export V46_43_HN_DPO_STEPS="${V46_43_HN_DPO_STEPS:-1800}"
export V46_43_HN_DPO_KINETIC_WEIGHT="${V46_43_HN_DPO_KINETIC_WEIGHT:-0.22}"
export V46_43_HN_DPO_MOTION_DENSITY_WEIGHT="${V46_43_HN_DPO_MOTION_DENSITY_WEIGHT:-0.18}"

# Compatibility: V46.42 patch still reads these variables because V46.43 is applied on top.
...
RUN_ROOT="${RUN_ROOT:-output/v46_43_physics_consistent_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "$RUN_ROOT/logs"
export V46_41_HN_DPO_DIR="${V46_41_HN_DPO_DIR:-$RUN_ROOT/v46_43_hn_pairs}"
...
echo "[V46.43 RUN_ROOT] $RUN_ROOT"
echo "[V46.43 AUDIO] $AUDIO"
...
echo "[2/14] Apply V46.38 routing patch + V46.41 TGT + V46.43 stability fixes"
...
python tools/apply_v46_43_physics_consistent_stability_patch.py
python -m py_compile \
...
python tools/v46_38_build_aesd_event_semantics.py \
  --db "$RAW_DB" \
  --out "$AESD_DB" \
  --json "$RUN_ROOT/V46_43_AESD_AUDIT.json"

echo "[5/14] Build strict final MSSD"
MSSD_JSON="$RUN_ROOT/${AUDIO_STEM}_v46_43_mssd.json"
...
CONTRASTIVE="$RUN_ROOT/v44_v46_43_mssd_aesd.pt"
REFINER="$RUN_ROOT/v45_v46_43_refiner.pt"
DIFFUSION="$RUN_ROOT/v46_v46_43_diffusion.pt"
...
python tools/v46_motionrag_diff.py --config "$CFG" generate \
  ...
  --out "$RUN_ROOT/${AUDIO_STEM}_v46_43_motionrag.npy" \
  --json "$RUN_ROOT/${AUDIO_STEM}_v46_43_motionrag.report.json" \
  --render_output "$RUN_ROOT/${AUDIO_STEM}_v46_43_motionrag.mp4"
...
python tools/v46_motionrag_diff.py --config "$CFG" generate \
  ...
  --out "$RUN_ROOT/${AUDIO_STEM}_v46_43_refiner_ik.npy" \
  --json "$RUN_ROOT/${AUDIO_STEM}_v46_43_refiner_ik.report.json" \
  --render_output "$RUN_ROOT/${AUDIO_STEM}_v46_43_refiner_ik.mp4"
...
python tools/v46_motionrag_diff.py --config "$CFG" generate \
  ...
  --out "$RUN_ROOT/${AUDIO_STEM}_v46_43_diffusion_ik.npy" \
  --json "$RUN_ROOT/${AUDIO_STEM}_v46_43_diffusion_ik.report.json" \
  --render_output "$RUN_ROOT/${AUDIO_STEM}_v46_43_diffusion_ik.mp4"

echo "[12/14] Verify routing + V46.41 TGT/KBO safety"
python tools/v46_38_verify_routing_report.py \
  --report "$RUN_ROOT/${AUDIO_STEM}_v46_43_diffusion_ik.report.json" \
  --mssd "$MSSD_JSON"
...
python tools/v46_42_extract_hn_dpo_pairs.py \
  ...
  --out_jsonl "$RUN_ROOT/V46_43_HN_DPO_PAIRS.jsonl" || true

if [[ "${V46_43_ENABLE_HN_DPO_FINETUNE:-0}" == "1" && -s "$RUN_ROOT/V46_43_HN_DPO_PAIRS.jsonl" ]]; then
  echo "[13b] Optional HN-DPO-style diffusion fine-tune"
  python tools/v46_43_train_hn_dpo_diffusion.py \
    --config "$CFG" \
    --base_diffusion "$DIFFUSION" \
    --pairs_jsonl "$RUN_ROOT/V46_43_HN_DPO_PAIRS.jsonl" \
    --steps "$V46_43_HN_DPO_STEPS" \
    --kinetic_weight "$V46_43_HN_DPO_KINETIC_WEIGHT" \
    --motion_density_weight "$V46_43_HN_DPO_MOTION_DENSITY_WEIGHT" \
    --kinetic_floor_ratio "${V46_43_HN_DPO_KINETIC_FLOOR_RATIO:-0.80}" \
    --out "$RUN_ROOT/v46_v46_43_diffusion_hn_dpo_kinetic.pt"
fi
...
for suffix in ["motionrag", "refiner_ik", "diffusion_ik"]:
    p=os.path.join(run, f"{stem}_v46_43_{suffix}.report.json")
    if os.path.exists(p):
        ...
        summary["outputs"][suffix]={
            ...
            "v46_43_stability_alignment": r.get("v46_43_stability_alignment", {}),
            "routing_policy_sample": (r.get("stage_reports", {}).get("retrieval", [{}]) or [{}])[0].get("routing_policy", ""),
        }
out=os.path.join(run, "V46_43_FINAL_SUMMARY.json")
json.dump(summary, open(out, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
print(json.dumps(summary, ensure_ascii=False, indent=2))
print("[SUMMARY]", out)
PY

echo "[DONE] V46.43 Stage-Anchored Guided TGT finished: $RUN_ROOT"
