...
python -m py_compile \
  ...
  tools/v40_native_floor_audit.py \
...
# V40: annotate / filter source-aware index by native foot-floor pathology before hierarchy build.
if [[ "${V40_NATIVE_FLOOR_AUDIT:-1}" == "1" ]]; then
  V40_FLOOR_JSON="${V40_FLOOR_JSON:-data/v40_source_aware/v34_shared_event_index_source_aware_floorclean.json}"
  V40_FLOOR_AUDIT="${V40_FLOOR_AUDIT:-data/v40_source_aware/v34_shared_event_index_source_aware_floorclean.audit.json}"
  python tools/v40_native_floor_audit.py     --index_json "$SOURCE_AWARE_JSON"     --out_json "$V40_FLOOR_JSON"     --audit_json "$V40_FLOOR_AUDIT"     --search_root data     --search_root output     --filter_mode "${V40_NATIVE_FLOOR_FILTER_MODE:-quality}"
  SOURCE_AWARE_JSON="$V40_FLOOR_JSON"
  SOURCE_AWARE_HIER_NPZ="${SOURCE_AWARE_HIER_NPZ:-data/v40_source_aware/v34_hierarchical_event_index_source_aware_floorclean.npz}"
  SOURCE_AWARE_HIER_JSON="${SOURCE_AWARE_HIER_JSON:-data/v40_source_aware/v34_hierarchical_event_index_source_aware_floorclean.json}"
fi
...
export V34_KINEMATIC_CONTACT_INFER="${V34_KINEMATIC_CONTACT_INFER:-1}"
export V34_KIN_CONTACT_HEIGHT="${V34_KIN_CONTACT_HEIGHT:-0.045}"
export V34_KIN_CONTACT_SPEED="${V34_KIN_CONTACT_SPEED:-0.035}"
export V34_FLOOR_CLEARANCE_POSTPROCESS="${V34_FLOOR_CLEARANCE_POSTPROCESS:-1}"
export V34_FLOOR_MARGIN="${V34_FLOOR_MARGIN:-0.006}"
export V34_FLOOR_STRENGTH="${V34_FLOOR_STRENGTH:-0.95}"
export V34_FLOOR_SMOOTH_WINDOW="${V34_FLOOR_SMOOTH_WINDOW:-5}"
export V34_FLOOR_MAX_LIFT="${V34_FLOOR_MAX_LIFT:-0.12}"
...
echo "[V40 SOURCE-AWARE RAG] RUN_ROOT=$RUN_ROOT"
echo "[V40 SOURCE-AWARE RAG] V34_INDEX_JSON=$V34_INDEX_JSON"
echo "[V40 SOURCE-AWARE RAG] V26_DURATION_INDEX_NPZ=$V26_DURATION_INDEX_NPZ"
echo "[V40 SOURCE-AWARE RAG] V26_HIERARCHY_INDEX_NPZ=$V26_HIERARCHY_INDEX_NPZ"
