...
OUT="${V26_OUT_DIR:-output/v31_safe_whole_song_$(date +%Y%m%d_%H%M%S)}"
ARGS=()
...
export V31_TRANSITION_SEED="${V31_TRANSITION_SEED:-20260610}"
...
export V31_MAX_TOTAL_RISK_RATIO="${V31_MAX_TOTAL_RISK_RATIO:-1.02}"
export V31_MAX_BOUNDARY_RATIO="${V31_MAX_BOUNDARY_RATIO:-1.04}"
export V31_MAX_JERK_RATIO="${V31_MAX_JERK_RATIO:-1.03}"
export V31_MAX_FOOT_RATIO="${V31_MAX_FOOT_RATIO:-1.05}"
export V31_MAX_ROTATION_STEP_RAD="${V31_MAX_ROTATION_STEP_RAD:-0.22}"
export V31_ENABLE_EDGE_DAMPING="${V31_ENABLE_EDGE_DAMPING:-0}"
export V31_STRICT_LOCKED_WARP="${V31_STRICT_LOCKED_WARP:-1}"
export V31_MAX_WARP_VIOLATIONS="${V31_MAX_WARP_VIOLATIONS:-0}"
...
python tools/schedule_v31_whole_song.py \
  ...
  --deep_music_cache "${V27_DEEP_MUSIC_CACHE:-data/v31_deep_music_features}" \
  ...
  --min_time_warp "${V26_MIN_TIME_WARP:-0.92}" \
  --max_time_warp "${V26_MAX_TIME_WARP:-1.22}" \
  --transition_min_frames "${V26_TRANSITION_MIN_FRAMES:-12}" \
  --transition_max_frames "${V26_TRANSITION_MAX_FRAMES:-42}" \
  --transition_diffusion "${V27_TRANSITION_DIFFUSION:-1}" \
  --transition_diffusion_blend "${V31_RESIDUAL_TRUST:-0.20}" \
  --transition_diffusion_steps "${V31_INFERENCE_STEPS:-36}" \
  ...
  --boundary_velocity_penalty_weight "${V26_BOUNDARY_VELOCITY_PENALTY_WEIGHT:-1.00}" \
  --boundary_acceleration_penalty_weight "${V26_BOUNDARY_ACCELERATION_PENALTY_WEIGHT:-1.00}" \
...
echo "[PASS] V31 whole-song output: $OUT"
