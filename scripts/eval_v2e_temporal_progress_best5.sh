...
OUT_ROOT=${OUT_ROOT:-output/stationary_v2e_best5_temporal_progress_eval}
MUSIC=${MUSIC:-test_music_bank/dunhuangwu2.wav}
UNITS=${UNITS:-"55370 61437 69373 70219 49122"}
STEPS=${STEPS:-"100 200 300 400"}
mkdir -p "$OUT_ROOT"
...
PY
    python generate_controlled.py \
      ...
      --sampler ddim \
      --guidance_weight 1.0 \
      --endpoint_keyframe_strength 0.75 \
      --mid_keyframe_strength 0.10 \
      --infer_keyframe_width 2 \
      --disable_traj_cond \
      --no_tto \
      --trajectory "0,0;0,0" \
      --mixed_precision bf16 \
      2>&1 | tee "logs/eval_v2e_${EXP_NAME}_e${step}_unit_${unit}.log"
    ...
    python render_from_npy.py \
      ...
      --camera_mode fixed \
      2>&1 | tee "logs/render_v2e_${EXP_NAME}_e${step}_unit_${unit}.log"
  done
  python tools/diagnose_endpoint_collapse_units.py \
    --pred_dir "$OUT" \
    --out_csv "$OUT_ROOT/endpoint_collapse_diag_e${step}.csv" \
    2>&1 | tee "logs/endpoint_collapse_diag_v2e_${EXP_NAME}_e${step}.log"
done
