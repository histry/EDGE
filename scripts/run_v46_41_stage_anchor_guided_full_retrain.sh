...
RUN_ROOT="${RUN_ROOT:-output/v46_41_stage_anchor_guided_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "$RUN_ROOT/logs"
export V46_41_HN_DPO_DIR="${V46_41_HN_DPO_DIR:-$RUN_ROOT/v46_41_hn_pairs}"
...
echo "[V46.41 RUN_ROOT] $RUN_ROOT"
echo "[V46.41 AUDIO] $AUDIO"
...
echo "[2/14] Apply V46.38 routing patch + V46.41 stage-anchor guided TGT patch"
...
python -m py_compile \
  ...
  tools/v46_41_verify_tgt_kbo_report.py \
  tools/v46_41_extract_hn_dpo_pairs.py \
...
python tools/v46_38_build_aesd_event_semantics.py \
  --db "$RAW_DB" \
  --out "$AESD_DB" \
  --json "$RUN_ROOT/V46_41_AESD_AUDIT.json"

echo "[5/14] Build strict final MSSD"
MSSD_JSON="$RUN_ROOT/${AUDIO_STEM}_v46_41_mssd.json"
...
CONTRASTIVE="$RUN_ROOT/v44_v46_41_mssd_aesd.pt"
REFINER="$RUN_ROOT/v45_v46_41_refiner.pt"
DIFFUSION="$RUN_ROOT/v46_v46_41_diffusion.pt"
...
python tools/v46_motionrag_diff.py --config "$CFG" generate \
  ...
  --out "$RUN_ROOT/${AUDIO_STEM}_v46_41_motionrag.npy" \
  --json "$RUN_ROOT/${AUDIO_STEM}_v46_41_motionrag.report.json" \
  --render_output "$RUN_ROOT/${AUDIO_STEM}_v46_41_motionrag.mp4"
...
python tools/v46_motionrag_diff.py --config "$CFG" generate \
  ...
  --out "$RUN_ROOT/${AUDIO_STEM}_v46_41_refiner_ik.npy" \
  --json "$RUN_ROOT/${AUDIO_STEM}_v46_41_refiner_ik.report.json" \
  --render_output "$RUN_ROOT/${AUDIO_STEM}_v46_41_refiner_ik.mp4"
...
python tools/v46_motionrag_diff.py --config "$CFG" generate \
  ...
  --out "$RUN_ROOT/${AUDIO_STEM}_v46_41_diffusion_ik.npy" \
  --json "$RUN_ROOT/${AUDIO_STEM}_v46_41_diffusion_ik.report.json" \
  --render_output "$RUN_ROOT/${AUDIO_STEM}_v46_41_diffusion_ik.mp4"

echo "[12/14] Verify routing + V46.41 TGT/KBO safety"
python tools/v46_38_verify_routing_report.py \
  --report "$RUN_ROOT/${AUDIO_STEM}_v46_41_diffusion_ik.report.json" \
  --mssd "$MSSD_JSON"
python tools/v46_41_verify_tgt_kbo_report.py \
  --report "$RUN_ROOT/${AUDIO_STEM}_v46_41_diffusion_ik.report.json" \
  --require_v46_38_routing \
  --require_v46_41_tokens \
  --out "$RUN_ROOT/V46_41_TGT_KBO_VERIFY.json"

echo "[13/14] Extract hard-negative DPO-style preference pairs"
python tools/v46_41_extract_hn_dpo_pairs.py \
  --report "$RUN_ROOT/${AUDIO_STEM}_v46_41_diffusion_ik.report.json" \
  --pair_dir "$V46_41_HN_DPO_DIR" \
  --out_jsonl "$RUN_ROOT/V46_41_HN_DPO_PAIRS.jsonl" || true

if [[ "${V46_41_ENABLE_HN_DPO_FINETUNE:-0}" == "1" && -s "$RUN_ROOT/V46_41_HN_DPO_PAIRS.jsonl" ]]; then
  echo "[13b] Optional HN-DPO-style diffusion fine-tune"
  python tools/v46_41_train_hn_dpo_diffusion.py \
    --config "$CFG" \
    --base_diffusion "$DIFFUSION" \
    --pairs_jsonl "$RUN_ROOT/V46_41_HN_DPO_PAIRS.jsonl" \
    --steps "${V46_41_HN_DPO_STEPS:-1500}" \
    --out "$RUN_ROOT/v46_v46_41_diffusion_hn_dpo.pt"
fi
...
for suffix in ["motionrag", "refiner_ik", "diffusion_ik"]:
    p=os.path.join(run, f"{stem}_v46_41_{suffix}.report.json")
    if os.path.exists(p):
...
out=os.path.join(run, "V46_41_FINAL_SUMMARY.json")
json.dump(summary, open(out, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
print(json.dumps(summary, ensure_ascii=False, indent=2))
print("[SUMMARY]", out)
PY

echo "[DONE] V46.41 Stage-Anchored Guided TGT finished: $RUN_ROOT"
