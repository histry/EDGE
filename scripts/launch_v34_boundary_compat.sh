...
# Launch V34 with boundary-compatible Event-RAG retrieval.
#
...
if [[ "${V34_TRAIN:-0}" == "1" ]]; then
  export V34_TRAIN=1
  export RUN_ID="${RUN_ID:-v34_boundary_compat_full_$(date +%Y%m%d_%H%M%S)}"
  bash scripts/launch_v34_full_overnight.sh "$@"
else
  export V34_TRAIN=0
  export V34_FAIL_ON_UNSAFE_BOUNDARY="${V34_FAIL_ON_UNSAFE_BOUNDARY:-0}"
  export RUN_ID="${RUN_ID:-v34_boundary_compat_infer_$(date +%Y%m%d_%H%M%S)}"
