...
1. 音乐条件能力继承自原 EDGE/AIST 预训练模型中的 audio-conditioned denoising prior。
2. 敦煌阶段主要学习敦煌姿态风格、关键帧过渡、轨迹控制和物理连续性。
3. proxy music 不作为真实标签，只作为节奏弱条件和接口占位。
4. 推理阶段从测试音乐中提取 onset/beat，作为节拍锚点参与轨迹节奏和可选姿态锚点引导。
